...
#ifndef UI_WINDOWS_INI
#define UI_WINDOWS_INI ".windows.ini" // editor_path("windows.ini")
#endif
...
static int ui_window_register(const char *panel_or_window_title) {
    ...
    if( *state == 0 ) {
        static ini_t i = 0;
        do_once i = ini(UI_WINDOWS_INI); // @leak
...
if( show_window_menu ) {
    ...
    if( ui_results.x == 1 ) {
        ...
        else if( clicked_save_layout ) file_delete(UI_WINDOWS_INI), ui_layout_all_save_disk("*");
...
int ui_layout_all_save_disk(const char *mask) {
    ...
    for each_map_ptr_sorted(ui_windows, char*, k, unsigned, v) {
        ...
        if( win && strmatchi(*k, mask) ) {
            ini_write(UI_WINDOWS_INI, *k, "x", va("%f", win->bounds.x / w ));
            ini_write(UI_WINDOWS_INI, *k, "y", va("%f", win->bounds.y / h ));
            ini_write(UI_WINDOWS_INI, *k, "w", va("%f", win->bounds.w / w ));
            ini_write(UI_WINDOWS_INI, *k, "h", va("%f", win->bounds.h / h ));
            ini_write(UI_WINDOWS_INI, *k, "visible", ui_visible(*k) ? "1":"0");
        }
    }
    return 1;
}
...
int ui_layout_all_load_disk(const char *mask) {
    ini_t i = ini(UI_WINDOWS_INI); // @leak
    if( !i ) return 0;
...
if(!win) { // if newly created window (!win)
    // first time, try to restore from UI_WINDOWS_INI file
    static ini_t i; do_once i = ini(UI_WINDOWS_INI); // @leak

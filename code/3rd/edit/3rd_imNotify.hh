...
struct ImGuiToast
{
    ...
    const char* getIcon() 
    {
        ...
        case ImGuiToastType::None:
            return nullptr;
        case ImGuiToastType::Success:
            return ICON_MD_CHECK_CIRCLE; //  "(v)" ICON_FA_CIRCLE_CHECK; // Font Awesome 6
        case ImGuiToastType::Warning:
            return ICON_MD_WARNING;      // "/!\\" ICON_FA_TRIANGLE_EXCLAMATION; // Font Awesome 6
        case ImGuiToastType::Error:
            return ICON_MD_ERROR;        //  "(!)" ICON_FA_CIRCLE_EXCLAMATION; // Font Awesome 6
        case ImGuiToastType::Info:
            return ICON_MD_SMS;          //  "(i)" ICON_FA_CIRCLE_INFO; // Font Awesome 6
        default:
            return nullptr;
        }
    }
        ...
        for (size_t i = 0; i < notifications.size(); ++i)
        {
            ...
            // Here we render the toast content
            {
                ...
                if (NOTIFY_USE_DISMISS_BUTTON)
                {
                    ...
                    if (Button(ICON_MD_CLOSE)) // "X")) //ICON_FA_XMARK))
                    {
                        notifications.erase(notifications.begin()+i);
                        --i;
                    }
...
void igInsertNotification(char type, const char *title, const char *message, float timeout) {
    ...
    ImGuiToast t = {
        ...
        title, message, (int)(timeout * 1000) };

...
//  style.Colors[ImGuiCol_ColumnActive]          = MED( 1.00f);
    ...
    style.SeparatorTextAlign.x = 0.50f;
...
int interval = 1; // 0 for immediate updates, 1 for updates synchronized with the vertical retrace, -1 for adaptive vsync
...
PRINTF("Monitor: %s (%.2fHz, vsync=%d)\n", SDL_GetDisplayName(monitor), SDL_GetCurrentDisplayMode(monitor)->refresh_rate, interval);
...
#if 1
ImFontConfig bright;
bright.RasterizerMultiply = 1.2f;
// Roboto-Medium 16
// ImFont* font = io.Fonts->AddFontFromFileTTF("art/fonts/OpenSans-Regular.ttf", 17.0f);
#if defined(_WIN32)
ImFont* font = io.Fonts->AddFontFromFileTTF("C:/Windows/Fonts/Arial.ttf", 16.0f, &bright);
#elif defined(__linux__)
ImFont* font = io.Fonts->AddFontFromFileTTF("/usr/share/fonts/liberation/LiberationSans-Regular.ttf", 16.0f, &bright);
#endif

int theFont_size;

for( char *theFont = file_read("MaterialIconsSharpRegular.otf", &theFont_size); theFont; theFont = 0 ) {
    ImFontConfig cfg;
    ...
    cfg.GlyphOffset.y = 8;            // 0, 0     // Offset all glyphs from this font input.
    ...
    static const ImWchar runes[] = {
        ICON_MD_MIN, ICON_MD_MAX_16, 0,
    };
    ImFont* font2 = io.Fonts->AddFontFromMemoryTTF(theFont, theFont_size, 24.f, &cfg, runes);
}

for( char *theFont = file_read("MaterialDesignIconsWebfont.ttf", &theFont_size); theFont; theFont = 0 ) {
    ImFontConfig cfg;
    cfg.FontDataOwnedByAtlas = false; // TTF/OTF data ownership taken by the container ImFontAtlas (will delete memory itself).
    cfg.OversampleH = 2; // Rasterize at higher quality for sub-pixel positioning. Note the difference between 2 and 3 is minimal. You can reduce this to 1 for large glyphs save memory. Read https://github.com/nothings/stb/blob/master/tests/oversample/README.md for details.
    cfg.OversampleV = 1; // Rasterize at higher quality for sub-pixel positioning. This is not really useful as we don't use sub-pixel positions on the Y axis.
    cfg.PixelSnapH = false;    // Align every glyph to pixel boundary. Useful e.g. if you are merging a non-pixel aligned font with the default font. If enabled, you can set OversampleH/V to 1.
    // ImVec2          GlyphExtraSpacing;      // 0, 0     // Extra spacing (in pixels) between glyphs. Only X axis is supported for now.
    cfg.GlyphOffset.y = 6;            // 0, 0     // Offset all glyphs from this font input.
    // const ImWchar*  GlyphRanges;            // NULL     // THE ARRAY DATA NEEDS TO PERSIST AS LONG AS THE FONT IS ALIVE. Pointer to a user-provided list of Unicode range (2 value per range, values are inclusive, zero-terminated list).
    // float           GlyphMinAdvanceX;       // 0        // Minimum AdvanceX for glyphs, set Min to align font icons, set both Min/Max to enforce mono-space font
    // float           GlyphMaxAdvanceX;       // FLT_MAX  // Maximum AdvanceX for glyphs
    cfg.MergeMode = true; // Merge into previous ImFont, so you can combine multiple inputs font into one ImFont (e.g. ASCII font + icons + Japanese glyphs). You may want to use GlyphOffset.y when merge font of different heights.
    ...
    // ImWchar         EllipsisChar;           // -1       // Explicitly specify unicode codepoint of ellipsis character. When fonts are being merged first specified ellipsis will be used.
    static const ImWchar runes[] = {
        0xF68C /*ICON_MDI_MIN*/, 0xF1CC7/*ICON_MDI_MAX*/, 0,
    };
    ImFont* font2 = io.Fonts->AddFontFromMemoryTTF(theFont, theFont_size, 24.f, &cfg, runes);
}
